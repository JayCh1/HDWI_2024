	! FORTRAN code to read GEFS forecast files and compute HDWI for a single day for CONUS
	!     Assumes we are using 12, 15, 18, 21, 00, and 03 times for CONUS
	!
	...
	!    gfortran -fopenmp HDW_Calc_GEFSfcst_jay.f -o HDW_Calc_GEFSfcst_jay.exe ${MOD} ${LIB}

        use wgrib2api
	character (len=100) :: in(6), inv(6)
	integer iret(6)
	...
	integer :: lev, nx, ny, i, j, nxt, nyt

	!  First, read in all the necesary variables from GEFS files for 1200, 1500, 1800, 2100, 0000, and 0300 
	!     (next day)
	!  tinker with the allocatable statements above and skip down to @@@@@@ if you are using other data 
	!     sources for:
	...
	!  All of the sfc/vertical variables should be on the same grid.
        ...
        iret(4) = grb2_mk_inv(in(4), inv(4))
        iret(5) = grb2_mk_inv(in(5), inv(5))
        iret(6) = grb2_mk_inv(in(6), inv(6))

        iret(1) = grb2_inq(in(1), inv(1), ':PRES:', ':surface:', 
     1        data2=tmp, nx=nx, ny=ny, lat=lat, lon=lon)

	allocate(psfc(6,nx,ny))
	allocate(tsfc(6,nx,ny))
	allocate(rsfc(6,nx,ny))
	allocate(usfc(6,nx,ny))
	allocate(vsfc(6,nx,ny))
	...
	allocate(praw(6,nx,ny,nlevst))
	allocate(psfc3d(6,nx,ny,nlevst))
	allocate(traw(6,nx,ny,nlevst))
	allocate(rraw(6,nx,ny,nlevst))
	allocate(uraw(6,nx,ny,nlevst))
	allocate(vraw(6,nx,ny,nlevst))
	...
	!         code could crash hard.
        ...
        ! note that GEFS ncrements starting at 1 at the south pole and at the international date line
        ! Dj :   0.5
        ! Di :   0.5
        ! Lat1 :  20
        ! Lat2 :  60
	...
	allocate(thraw(6,nxt,nyt,nlevst))
	allocate(svpraw(6,nxt,nyt,nlevst))
	allocate(essfc(6,nxt,nyt))
	allocate(esraw(6,nxt,nyt,nlevst))
	allocate(esfc(6,nxt,nyt))
	allocate(eraw(6,nxt,nyt,nlevst))
	allocate(vpdsfc(6,nxt,nyt))
	allocate(vpdraw(6,nxt,nyt,nlevst))
	allocate(wsfc(6,nxt,nyt))
	allocate(wraw(6,nxt,nyt,nlevst))
	allocate(ptop0(6,nxt,nyt))
	...
	! allocate the final HDWI variables
        allocate(wmax0(6,nxt,nyt))
        allocate(vpdmax0(6,nxt,nyt))
        allocate(hdw0(6,nxt,nyt))
        allocate(hdwmax0(nxt,nyt))
        allocate(vpdmaxout(nxt,nyt))
        allocate(wmaxout(nxt,nyt))

        ! allocate trimmed versions of certain variables
        allocate(tsfct(6,nxt,nyt))
        allocate(psfct(6,nxt,nyt))
        allocate(prawt(6,nxt,nyt,nlevst))
